...
import { deleteBooking, deleteBookings, toggleBooking, toggleVipAvailability } from "./actions";
    ...
    <main className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14">
      ...
      <section className="mt-9 rounded-2xl border border-slate-200 bg-white p-6" aria-labelledby="vip-control-heading">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-3">
          {plans.map((plan) => <article key={plan.id} className="rounded-xl border border-slate-200 p-4 text-center"><h3 className="font-black text-slate-950">{plan.name}</h3><p className={`mt-2 text-xs font-black ${plan.isSoldOut ? "text-red-600" : "text-emerald-600"}`}>{plan.isSoldOut ? "Sold Out" : "Available"}</p><form action={toggleVipAvailability}><input type="hidden" name="id" value={plan.id} /><input type="hidden" name="isSoldOut" value={String(!plan.isSoldOut)} /><button className={`mt-3 min-h-10 w-full rounded-lg px-3 text-xs font-black text-white ${plan.isSoldOut ? "bg-emerald-600" : "bg-red-500"}`}>Mark {plan.isSoldOut ? "Available" : "Sold Out"}</button></form></article>)}
        </div>
      </section>
