...
export async function updatePlan(formData: FormData) {
  ...
  const input = readPlan(formData);
  await getDatabase().plan.update({ where: { id }, data: { name: input.name, description: input.description, priceMinor: Math.round(input.price * 100), durationDays: input.durationDays, scope: input.scope, deckId: input.scope === "DECK" ? input.deckId : null, sortOrder: input.sortOrder, isActive: input.isActive } });
  await recordAudit({ actorId: actor.id, action: "PLAN_UPDATED", entityType: "Plan", entityId: id, metadata: { priceMinor: Math.round(input.price * 100), durationDays: input.durationDays, isActive: input.isActive } });
  revalidatePath("/vip"); revalidatePath("/admin/plans");
}
