...
export function SlipLoaderForm({ defaultDate }: { defaultDate: string }) {
  ...
  return (
    <form ref={formRef} action={action} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
      ...
      <p className="mt-2 text-sm leading-6 text-slate-500">Enter one booking code and choose where its matches should appear. Tips Deck imports the full slip automatically.</p>
      <div className="mt-6 grid gap-5 sm:grid-cols-3">
        ...
        <label className="text-sm font-bold text-slate-700 sm:col-span-2">Category<select name="category" defaultValue="FREE" className={input}><option value="FREE">Free Predictions</option><option value="VIP1">VIP 1 Predictions</option><option value="VIP2">VIP 2 Predictions</option><option value="VIP3">VIP 3 Predictions</option></select></label>
