...
export function AccountPreview({ mode, next = "/account" }: { mode: "login" | "register"; next?: string }) {
  ...
  return (
    <main className="bg-[#f7f9f8] px-5 py-8 sm:px-8 sm:py-12">
      <div className="mx-auto max-w-md rounded-2xl border border-slate-200 bg-white p-7 shadow-[0_18px_50px_rgba(15,23,42,0.08)] sm:p-9">
        ...
        <form action={action} className="mt-7 grid gap-4">
          ...
          {register && <><AuthField label="Confirm Password" name="confirmPassword" required type="password" autoComplete="new-password" placeholder="Confirm your password" icon="lock" /><label className="flex items-start gap-3 text-sm leading-6 text-slate-600"><input name="termsAccepted" type="checkbox" required className="mt-1 size-4 shrink-0 accent-emerald-600" /><span>I agree to the <Link href="/terms" className="font-bold text-emerald-700 hover:underline">Terms of Service</Link> and <Link href="/privacy" className="font-bold text-emerald-700 hover:underline">Privacy Policy</Link>.</span></label></>}
