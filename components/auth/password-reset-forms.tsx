...
function AuthShell({ title, copy, children }: { title: string; copy: string; children: React.ReactNode }) {
  return <main className="bg-[#f7f9f8] px-5 py-8 sm:px-8 sm:py-12"><div className="mx-auto max-w-md rounded-2xl border border-slate-200 bg-white p-7 shadow-[0_18px_50px_rgba(15,23,42,0.08)] sm:p-9"><Link href="/" aria-label="Tips Deck home" className="mx-auto mb-6 flex w-fit justify-center rounded-lg focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-emerald-600"><Image src="/brand/tips-deck-lockup.png" alt="Tips Deck" width={180} height={78} priority className="h-16 w-auto object-contain" /></Link><h1 className="text-3xl font-black tracking-[-0.04em] text-slate-950">{title}</h1><p className="mt-2 text-sm leading-6 text-slate-500">{copy}</p>{children}<p className="mt-6 text-center"><Link href="/login" className="text-sm font-black text-emerald-700">Back to Login</Link></p></div></main>;
}
